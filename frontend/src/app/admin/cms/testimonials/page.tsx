...
export default function TestimonialsPage() {
  ...
  const [form, setForm] = useState({ name: "", role: "", company: "", content: "", avatarUrl: "", rating: 5, status: "draft" })

  const fetchItems = () => {
    setLoading(true)
    cmsApi.testimonials.getAll()
      .then(setItems)
      .catch(() => {})
  ...
  const openCreate = () => {
    setEditing(null)
    setForm({ name: "", role: "", company: "", content: "", avatarUrl: "", rating: 5, status: "draft" })
    setDialogOpen(true)
  }

  const openEdit = (item: Testimonial) => {
    setEditing(item)
    setForm({ name: item.name, role: item.role, company: item.company, content: item.content, avatarUrl: item.avatarUrl || "", rating: item.rating, status: item.status })
    setDialogOpen(true)
  }

  const handleSave = async () => {
    if (editing) {
      await cmsApi.testimonials.update(editing.id, form)
    } else {
      await cmsApi.testimonials.create(form)
    }
    setDialogOpen(false)
    fetchItems()
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Create Testimonial</Button>
        </div>
      </div>

      <Card className="glass-card rounded-2xl p-6">
        <CardHeader className="px-0 pt-0"><CardTitle>All Testimonials</CardTitle></CardHeader>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={4} placeholder="Testimonial text..." />
            </div>
            <div>

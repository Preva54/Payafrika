...
import { cmsApi, type Service } from "@/lib/cms-api"
...
export default function ServicesPage() {
  ...
  const [form, setForm] = useState({ title: "", slug: "", description: "", icon: "", status: "draft" })

  const fetchItems = () => {
    setLoading(true)
    cmsApi.services.getAll()
      .then(setItems)
      .catch(() => {})
  ...
  const openCreate = () => {
    setEditing(null)
    setForm({ title: "", slug: "", description: "", icon: "", status: "draft" })
    setDialogOpen(true)
  }

  const openEdit = (item: Service) => {
    setEditing(item)
    setForm({ title: item.title, slug: item.slug, description: item.description, icon: item.icon, status: item.status })
    setDialogOpen(true)
  }

  const handleSave = async () => {
    if (editing) {
      await cmsApi.services.update(editing.id, form)
    } else {
      await cmsApi.services.create(form)
    }
    setDialogOpen(false)
    fetchItems()
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    await cmsApi.services.delete(id)
    fetchItems()
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Create Service</Button>
        </div>
      </div>

      <Card className="glass-card rounded-2xl p-6">
        <CardHeader className="px-0 pt-0"><CardTitle>All Services</CardTitle></CardHeader>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="service-slug" />
              </div>
            </div>
            <div>
              <label className="text-sm font-medium mb-1 block">Description</label>
              <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} placeholder="Service description..." />
            </div>
            <div>
